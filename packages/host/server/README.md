...
| `GET`  | `/mu/v1/lookup`                   | `Authorization: Bearer <apiKey>` + `X-Dsh-Tenant`                 | `?type={user\|workspace\|session}&biz_key=…&tenant_key=…`                                          | `{ ok: true, hit: { type, id, bizKey, tenantId, attrs, createdAt, updatedAt } }` or `{ ok: true, miss: true }` |
...
`attrs` is the free-form JSONB column landed by storage-postgres migration `0004_ensure_attributes.sql`. Hosts pass any plain object on insert; ensure is idempotent so passing `attrs` on a row that already exists is a no-op (the existing row's `attrs` is returned untouched).
